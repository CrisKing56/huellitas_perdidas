...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Refugios - Huellitas Perdidas</title>
    <style>
        @page { margin: 1cm; }
        ...
        <table>
            <tr>
                <td>
                    ...
                    <p class="subtitle">Reporte Administrativo de Refugios</p>
                </td>
                <td class="info-meta">
    ...
    <table>
        ...
        <tbody>
            @forelse($refugios as $refugio)
                <tr>
                    <td><strong>#{{ $refugio->id_organizacion }}</strong></td>
                    <td>{{ $refugio->nombre }}</td>
                    <td>
                        {{ $refugio->nombre_usuario ?? 'N/A' }}<br>
                        <small style="color: #64748b;">{{ $refugio->telefono }}</small>
                    </td>
                    <td>{{ $refugio->colonia }}, {{ $refugio->ciudad }}</td>
                    <td>
                        @php
                            $clase = match($refugio->estado_revision) {
                        ...
                        @endphp
                        <span class="badge {{ $clase }}">{{ $refugio->estado_revision }}</span>
                    </td>
                </tr>
            @empty
                <tr>
