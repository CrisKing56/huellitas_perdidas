...
<div class="min-h-screen bg-gradient-to-b from-orange-50/40 via-gray-50 to-white py-8 sm:py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-6 sm:px-8 pt-6 sm:pt-8 pb-5 bg-gradient-to-r from-white via-orange-50/40 to-white">
                        <div class="flex flex-wrap items-center gap-3 mb-4">
                            ...
                            @if($fechaPublicacion)
                                <span class="inline-flex items-center gap-2 text-sm text-gray-500 font-medium">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2 text-base sm:text-lg text-gray-500">
                            <span>
                            ...
                            @if($publicacion->raza)
                                ...
                                <span>{{ $publicacion->raza }}</span>
                            @endif
                        </div>
                    </div>

                    <div class="p-4 sm:p-5 pt-0">
                        <div class="relative w-full h-[320px] sm:h-[440px] bg-gray-200 rounded-[1.6rem] overflow-hidden border border-gray-100 group">
                            @if($publicacion->fotoPrincipal)
                                <img
                                    src="{{ asset('storage/' . $publicacion->fotoPrincipal->url) }}"
                                    alt="{{ $publicacion->nombre }}"
                                    class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                                >
                                <div class="pointer-events-none absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-black/35 via-black/10 to-transparent"></div>
                            @else
            ...
            <div class="lg:col-span-5">
                <div class="space-y-6 lg:sticky lg:top-24">
                    ...
                    {{-- CONTACTO + ACCIONES --}}
                    <div id="acciones-publicacion" class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-white to-orange-50/60">
                            <h3 class="text-lg font-bold text-gray-900">Información de contacto</h3>
                            <p class="text-sm text-gray-500 mt-1">Ponte en contacto con la persona responsable de la publicación.</p>
                        </div>

                        <div class="p-6">
                            <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                <div class="w-12 h-12 rounded-full bg-orange-500 text-white flex items-center justify-center font-bold text-lg shadow-sm flex-shrink-0">
                                    {{ mb_strtoupper(mb_substr($publicacion->autor->nombre ?? 'U', 0, 1)) }}
                                </div>

                                <div class="min-w-0">
                                    <p class="text-[11px] text-gray-500 font-bold uppercase tracking-wider">Dueño</p>
                                    <p class="font-bold text-gray-900 truncate">{{ $publicacion->autor->nombre ?? 'Usuario' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3">
                                @if($whatsappUrl)
                                    <a
                                        href="{{ $whatsappUrl }}"
                                        target="_blank"
                                        class="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold py-3.5 px-4 rounded-2xl shadow-md transition-all hover:-translate-y-0.5 flex justify-center items-center gap-2"
                                    >
                                ...
                                @else
                                    <button
                                        type="button"
                                        disabled
                                        class="w-full bg-gray-300 text-white font-bold py-3.5 px-4 rounded-2xl shadow-sm cursor-not-allowed flex justify-center items-center gap-2"
                                    >
                            ...
                            @if($mostrarBotonReporte)
                                <div class="mt-4">
                                    @guest
                                        <a
                                            href="{{ route('login') }}"
                                            class="w-full border border-red-200 bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-3.5 px-4 rounded-2xl transition flex justify-center items-center gap-2"
                                        >
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @auth
                                        <details class="group" {{ $abrirReporte ? 'open' : '' }}>
                                            <summary class="list-none cursor-pointer w-full border border-red-200 bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-3.5 px-4 rounded-2xl transition flex justify-center items-center gap-2">
                                                <svg class="w-4 h-4 transition-transform group-open:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636l-1.414-1.414a2 2 0 00-2.828 0L4 14.343V20h5.657L19.778 9.879a2 2 0 000-2.829z"></path>
                                                </svg>
                                                Reportar publicación
                                            </summary>

                                            <div class="mt-4 rounded-2xl border border-red-100 bg-red-50/50 p-4">
                                                <form action="{{ route('extravios.reportar', $publicacion->id_publicacion) }}" method="POST" class="space-y-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <select
                                                            name="motivo_id"
                                                            class="w-full border border-gray-200 rounded-2xl p-3 text-sm bg-white focus:ring-2 focus:ring-red-400 focus:border-transparent outline-none"
                                                        >
                                                    ...
                                                    <div>
                                                        ...
                                                        <textarea
                                                            ...
                                                            rows="4"
                                                            class="w-full border border-gray-200 rounded-2xl p-3 text-sm bg-white focus:ring-2 focus:ring-red-400 focus:border-transparent outline-none resize-none"
                            ...
                            <div class="mt-6 pt-6 border-t border-gray-100">
                                <p class="text-xs text-gray-500 mb-3 font-semibold uppercase tracking-wide">Acciones</p>

                                <div class="grid {{ $esAutor ? 'grid-cols-2' : 'grid-cols-1' }} gap-3">
                                    @if($esAutor)
                                        <button
                                            onclick="window.print()"
                                            class="flex items-center justify-center gap-2 py-3 px-4 bg-gray-50 hover:bg-gray-100 rounded-2xl text-gray-600 text-sm font-medium transition border border-gray-200"
                                        >
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <button
                                        class="flex items-center justify-center gap-2 py-3 px-4 bg-orange-50 hover:bg-orange-100 rounded-2xl text-orange-600 text-sm font-medium transition border border-orange-100"
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-8 mt-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gradient-to-r from-white to-orange-50/60">
                    <h3 class="text-xl font-bold text-gray-900">Información de la mascota</h3>
                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-4">
                                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">Fecha de extravío</p>
                            ...
                            <div class="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-4">
                                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">Último avistamiento</p>
                                <p class="text-gray-900 font-semibold mt-1">{{ $publicacion->colonia_barrio }}</p>
                        ...
                        <div>
                            <p class="text-xs text-gray-500 font-medium uppercase tracking-wide mb-3">Características</p>
                            <div class="flex flex-wrap gap-2">
                                <span class="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium border border-gray-200">
                                    Sexo: {{ ucfirst($publicacion->sexo) }}
                                </span>
                                <span class="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium border border-gray-200">
                                    Tamaño: {{ ucfirst($publicacion->tamano) }}
                                </span>
                                <span class="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium border border-gray-200">
                                    Color: {{ ucfirst($publicacion->color) }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gradient-to-r from-white to-orange-50/60">
                    <h3 class="text-xl font-bold text-gray-900">Última ubicación conocida</h3>
                </div>
                ...
                <div class="p-6 sm:p-8">
                    ...
                    @else
                        <div class="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                            <p class="text-red-500 font-medium">No hay coordenadas exactas para esta mascota.</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        {{-- COMENTARIOS --}}
        <div id="comentarios" class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mt-8">
            <div class="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gradient-to-r from-white to-orange-50/60">
            ...
            <div class="p-6 sm:p-8">
                ...
                @auth
                    <div class="mb-8 rounded-3xl border border-orange-100 bg-orange-50/60 p-4 sm:p-5">
                        <form action="{{ route('extravios.comentarios.store', $publicacion->id_publicacion) }}" method="POST">
                            ...
                            <div class="flex justify-end mt-3">
                                <button
                                    type="submit"
                                    class="bg-orange-600 text-white px-6 py-2.5 rounded-2xl text-sm font-semibold hover:bg-orange-700 transition flex items-center gap-2 shadow-sm"
                                >
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-6">
                    @forelse($comentarios as $comentario)
                        ...
                        <div class="rounded-3xl border border-gray-100 bg-gray-50/80 p-5 sm:p-6">
                                                    ...
                                                    <div class="mt-3">
                                                        <form action="{{ route('extravios.comentarios.store', $publicacion->id_publicacion) }}" method="POST" class="space-y-3">
                                                            ...
                                                            <div class="flex justify-end">
                                                                <button
                                                                    type="submit"
                                                                    class="bg-orange-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-orange-700 transition"
                                                                >
                                                                    Responder
                                                                </button>
                                                            </div>
    ...
    <div class="flex-grow flex flex-col justify-center items-center overflow-hidden min-h-0">
        ...
        <div class="w-full">
            ...
            <div class="text-left px-4">
                ...
                <div class="bg-yellow-50 p-3 rounded border border-yellow-200 text-sm italic text-gray-700 line-clamp-3">
                    "{{ $publicacion->descripcion }}"
                </div>
            </div>
        </div>
